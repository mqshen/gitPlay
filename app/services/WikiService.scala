...
trait WikiService {
  ...
  def createWikiRepository(loginAccount: models.User, owner: String, repository: String): Unit =
  ...
  def revertWikiPage(owner: String, repository: String, from: String, to: String,
                     committer: models.User, pageName: Option[String]): Boolean = {
  ...
  def saveWikiPage(owner: String, repository: String, currentPageName: String, newPageName: String,
                   content: String, committer: models.User, message: String, currentId: Option[String]): Option[String] = {
    LockUtil.lock(s"${owner}/${repository}/wiki"){
      using(Git.open(Directory.getWikiRepositoryDir(owner, repository))){ git =>
        val builder  = DirCache.newInCore.builder()
        val inserter = git.getRepository.newObjectInserter()
